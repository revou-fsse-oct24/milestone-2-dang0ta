 ...
 */
export async function getProducts({offset}: {offset: number}): Promise<Response<Product[]>> {
    try {
 ...
 */
export async function getProduct({id}: {id: string}): Promise<Response<Product>> {
    try {
        const res = await fetch(getProductsURL(parseInt(id)));
        if (!res.ok) {
            console.warn(res.statusText, id);
            return {status: "error", data: defaultProduct(), error: "An error occurred while fetching the data"}
        }
        ...
        const parsedErr = parseError(e)
        return {status: "error", data: defaultProduct(), error: parsedErr.message}
    }
}